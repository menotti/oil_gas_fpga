...
class su_gather {
  private:
    ...
    int _max_gather;           // max number of cdps in a gather
    std::vector<std::vector<int>> _cdps_by_cdp_id; // Cdps to be used in the evaluation of cdp with cdp_id

  public:
    su_gather(std::string& bin_file_path, int aph, int apm, int nc);
    ...
    inline int max_gather() { return _max_gather;     }
    inline const std::vector<std::vector<int>> cdps_by_cdp_id() const {return _cdps_by_cdp_id;}
