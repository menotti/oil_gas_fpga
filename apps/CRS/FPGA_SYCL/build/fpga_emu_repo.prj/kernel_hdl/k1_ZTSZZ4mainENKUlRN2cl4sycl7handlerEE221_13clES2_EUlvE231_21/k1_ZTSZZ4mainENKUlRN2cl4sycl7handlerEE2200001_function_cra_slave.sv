...
// SystemVerilog created from k1_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE2200001_function_cra_slave
...
module k1_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE2200001_function_cra_slave (
    ...
    output wire [2079:0] kernel_arguments,
    ...
    wire [2079:0] kernel_arg_bit_join_q;
    ...
    // printf_bit(BITSELECT,265)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,266)
    ...
    // readdata_valid_stage1_reg(REG,273)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,169)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // printf_reset_bit(BITSELECT,268)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,164)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,161)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // readdata_valid_stage2_reg(REG,274)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,162)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,185)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,179)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,187)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,181)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2_buffered(REG,190)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,189)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,183)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,194)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,196)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,198)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_30_arg_bit_join_x(BITJOIN,537)
    assign dupName_30_arg_bit_join_x_q = {arguments_64_q, arguments_63_q};

    // dupName_79_mask0_x(LOGICAL,735)
    assign dupName_79_mask0_x_q = bit_enable_bottom_bar_b & arguments_62_buffered_q;

    // dupName_79_new_data_x(LOGICAL,737)
    ...
    // dupName_29_arg_bit_join_x(BITJOIN,529)
    assign dupName_29_arg_bit_join_x_q = {arguments_62_q, arguments_61_q};

    // dupName_77_mask0_x(LOGICAL,729)
    assign dupName_77_mask0_x_q = bit_enable_bottom_bar_b & arguments_60_buffered_q;

    // dupName_77_new_data_x(LOGICAL,731)
    ...
    // dupName_28_arg_bit_join_x(BITJOIN,521)
    assign dupName_28_arg_bit_join_x_q = {arguments_60_q, arguments_59_q};

    // dupName_75_mask0_x(LOGICAL,723)
    assign dupName_75_mask0_x_q = bit_enable_bottom_bar_b & arguments_58_buffered_q;

    // dupName_75_new_data_x(LOGICAL,725)
    ...
    // dupName_27_arg_bit_join_x(BITJOIN,513)
    assign dupName_27_arg_bit_join_x_q = {arguments_58_q, arguments_57_q};

    // dupName_73_mask0_x(LOGICAL,717)
    assign dupName_73_mask0_x_q = bit_enable_bottom_bar_b & arguments_56_buffered_q;

    // dupName_73_new_data_x(LOGICAL,719)
    ...
    // dupName_26_arg_bit_join_x(BITJOIN,505)
    assign dupName_26_arg_bit_join_x_q = {arguments_56_q, arguments_55_q};

    // dupName_71_mask0_x(LOGICAL,711)
    assign dupName_71_mask0_x_q = bit_enable_bottom_bar_b & arguments_54_buffered_q;

    // dupName_71_new_data_x(LOGICAL,713)
    ...
    // dupName_25_arg_bit_join_x(BITJOIN,497)
    assign dupName_25_arg_bit_join_x_q = {arguments_54_q, arguments_53_q};

    // dupName_69_mask0_x(LOGICAL,705)
    assign dupName_69_mask0_x_q = bit_enable_bottom_bar_b & arguments_52_buffered_q;

    // dupName_69_new_data_x(LOGICAL,707)
    ...
    // dupName_24_arg_bit_join_x(BITJOIN,489)
    assign dupName_24_arg_bit_join_x_q = {arguments_52_q, arguments_51_q};

    // dupName_67_mask0_x(LOGICAL,699)
    assign dupName_67_mask0_x_q = bit_enable_bottom_bar_b & arguments_50_buffered_q;

    // dupName_67_new_data_x(LOGICAL,701)
    ...
    // dupName_23_arg_bit_join_x(BITJOIN,481)
    assign dupName_23_arg_bit_join_x_q = {arguments_50_q, arguments_49_q};

    // dupName_65_mask0_x(LOGICAL,693)
    assign dupName_65_mask0_x_q = bit_enable_bottom_bar_b & arguments_48_buffered_q;

    // dupName_65_new_data_x(LOGICAL,695)
    ...
    // dupName_22_arg_bit_join_x(BITJOIN,473)
    assign dupName_22_arg_bit_join_x_q = {arguments_48_q, arguments_47_q};

    // dupName_63_mask0_x(LOGICAL,687)
    assign dupName_63_mask0_x_q = bit_enable_bottom_bar_b & arguments_46_buffered_q;

    // dupName_63_new_data_x(LOGICAL,689)
    ...
    // dupName_21_arg_bit_join_x(BITJOIN,465)
    assign dupName_21_arg_bit_join_x_q = {arguments_46_q, arguments_45_q};

    // dupName_61_mask0_x(LOGICAL,681)
    assign dupName_61_mask0_x_q = bit_enable_bottom_bar_b & arguments_44_buffered_q;

    // dupName_61_new_data_x(LOGICAL,683)
    ...
    // dupName_20_arg_bit_join_x(BITJOIN,457)
    assign dupName_20_arg_bit_join_x_q = {arguments_44_q, arguments_43_q};

    // dupName_59_mask0_x(LOGICAL,675)
    assign dupName_59_mask0_x_q = bit_enable_bottom_bar_b & arguments_42_buffered_q;

    // dupName_59_new_data_x(LOGICAL,677)
    ...
    // dupName_19_arg_bit_join_x(BITJOIN,449)
    assign dupName_19_arg_bit_join_x_q = {arguments_42_q, arguments_41_q};

    // dupName_57_mask0_x(LOGICAL,669)
    assign dupName_57_mask0_x_q = bit_enable_bottom_bar_b & arguments_40_buffered_q;

    // dupName_57_new_data_x(LOGICAL,671)
    ...
    // dupName_18_arg_bit_join_x(BITJOIN,441)
    assign dupName_18_arg_bit_join_x_q = {arguments_40_q, arguments_39_q};

    // dupName_55_mask0_x(LOGICAL,663)
    assign dupName_55_mask0_x_q = bit_enable_bottom_bar_b & arguments_38_buffered_q;

    // dupName_55_new_data_x(LOGICAL,665)
    ...
    // dupName_17_arg_bit_join_x(BITJOIN,433)
    assign dupName_17_arg_bit_join_x_q = {arguments_38_q, arguments_37_q};

    // dupName_53_mask0_x(LOGICAL,657)
    assign dupName_53_mask0_x_q = bit_enable_bottom_bar_b & arguments_36_buffered_q;

    // dupName_53_new_data_x(LOGICAL,659)
    ...
    // dupName_16_arg_bit_join_x(BITJOIN,425)
    assign dupName_16_arg_bit_join_x_q = {arguments_36_q, arguments_35_q};

    // dupName_51_mask0_x(LOGICAL,651)
    assign dupName_51_mask0_x_q = bit_enable_bottom_bar_b & arguments_34_buffered_q;

    // dupName_51_new_data_x(LOGICAL,653)
    ...
    // dupName_15_arg_bit_join_x(BITJOIN,417)
    assign dupName_15_arg_bit_join_x_q = {arguments_34_q, arguments_33_q};

    // dupName_49_mask0_x(LOGICAL,645)
    assign dupName_49_mask0_x_q = bit_enable_bottom_bar_b & arguments_32_buffered_q;

    // dupName_49_new_data_x(LOGICAL,647)
    ...
    // dupName_14_arg_bit_join_x(BITJOIN,409)
    assign dupName_14_arg_bit_join_x_q = {arguments_32_q, arguments_31_q};

    // dupName_47_mask0_x(LOGICAL,639)
    assign dupName_47_mask0_x_q = bit_enable_bottom_bar_b & arguments_30_buffered_q;

    // dupName_47_new_data_x(LOGICAL,641)
    ...
    // dupName_13_arg_bit_join_x(BITJOIN,401)
    assign dupName_13_arg_bit_join_x_q = {arguments_30_q, arguments_29_q};

    // dupName_45_mask0_x(LOGICAL,630)
    assign dupName_45_mask0_x_q = bit_enable_bottom_bar_b & arguments_28_buffered_q;

    // dupName_45_new_data_x(LOGICAL,632)
    ...
    // dupName_12_arg_bit_join_x(BITJOIN,393)
    assign dupName_12_arg_bit_join_x_q = {arguments_28_q, arguments_27_q};

    // dupName_43_mask0_x(LOGICAL,618)
    assign dupName_43_mask0_x_q = bit_enable_bottom_bar_b & arguments_26_buffered_q;

    // dupName_43_new_data_x(LOGICAL,620)
    ...
    // dupName_11_arg_bit_join_x(BITJOIN,385)
    assign dupName_11_arg_bit_join_x_q = {arguments_26_q, arguments_25_q};

    // dupName_41_mask0_x(LOGICAL,606)
    assign dupName_41_mask0_x_q = bit_enable_bottom_bar_b & arguments_24_buffered_q;

    // dupName_41_new_data_x(LOGICAL,608)
    ...
    // dupName_10_arg_bit_join_x(BITJOIN,377)
    assign dupName_10_arg_bit_join_x_q = {arguments_24_q, arguments_23_q};

    // dupName_39_mask0_x(LOGICAL,594)
    assign dupName_39_mask0_x_q = bit_enable_bottom_bar_b & arguments_22_buffered_q;

    // dupName_39_new_data_x(LOGICAL,596)
    ...
    // dupName_9_arg_bit_join_x(BITJOIN,369)
    assign dupName_9_arg_bit_join_x_q = {arguments_22_q, arguments_21_q};

    // dupName_37_mask0_x(LOGICAL,582)
    assign dupName_37_mask0_x_q = bit_enable_bottom_bar_b & arguments_20_buffered_q;

    // dupName_37_new_data_x(LOGICAL,584)
    ...
    // dupName_8_arg_bit_join_x(BITJOIN,361)
    assign dupName_8_arg_bit_join_x_q = {arguments_20_q, arguments_19_q};

    // dupName_35_mask0_x(LOGICAL,570)
    assign dupName_35_mask0_x_q = bit_enable_bottom_bar_b & arguments_18_buffered_q;

    // dupName_35_new_data_x(LOGICAL,572)
    ...
    // dupName_7_arg_bit_join_x(BITJOIN,353)
    assign dupName_7_arg_bit_join_x_q = {arguments_18_q, arguments_17_q};

    // dupName_33_mask0_x(LOGICAL,558)
    assign dupName_33_mask0_x_q = bit_enable_bottom_bar_b & arguments_16_buffered_q;

    // dupName_33_new_data_x(LOGICAL,560)
    ...
    // dupName_6_arg_bit_join_x(BITJOIN,345)
    assign dupName_6_arg_bit_join_x_q = {arguments_16_q, arguments_15_q};

    // dupName_31_mask0_x(LOGICAL,546)
    assign dupName_31_mask0_x_q = bit_enable_bottom_bar_b & arguments_14_buffered_q;

    // dupName_31_new_data_x(LOGICAL,548)
    ...
    // dupName_5_arg_bit_join_x(BITJOIN,337)
    assign dupName_5_arg_bit_join_x_q = {arguments_14_q, arguments_13_q};

    // dupName_29_mask0_x(LOGICAL,532)
    assign dupName_29_mask0_x_q = bit_enable_bottom_bar_b & arguments_12_buffered_q;

    // dupName_29_new_data_x(LOGICAL,534)
    ...
    // dupName_4_arg_bit_join_x(BITJOIN,330)
    assign dupName_4_arg_bit_join_x_q = {arguments_12_q, arguments_11_q};

    // dupName_27_mask0_x(LOGICAL,516)
    assign dupName_27_mask0_x_q = bit_enable_bottom_bar_b & arguments_10_buffered_q;

    // dupName_27_new_data_x(LOGICAL,518)
    ...
    // dupName_3_arg_bit_join_x(BITJOIN,325)
    assign dupName_3_arg_bit_join_x_q = {arguments_10_q, arguments_9_q};

    // dupName_25_mask0_x(LOGICAL,500)
    assign dupName_25_mask0_x_q = bit_enable_bottom_bar_b & arguments_8_buffered_q;

    // dupName_25_new_data_x(LOGICAL,502)
    assign dupName_25_new_data_x_q = dupName_25_mask0_x_q | mask1_q;

    // arguments_8_buffered(REG,134)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_2_arg_bit_join_x(BITJOIN,320)
    assign dupName_2_arg_bit_join_x_q = {arguments_8_q, arguments_7_q};

    // dupName_23_mask0_x(LOGICAL,484)
    assign dupName_23_mask0_x_q = bit_enable_bottom_bar_b & arguments_6_buffered_q;

    // dupName_23_new_data_x(LOGICAL,486)
    assign dupName_23_new_data_x_q = dupName_23_mask0_x_q | mask1_q;

    // arguments_6_buffered(REG,130)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // kernel_arg_bit_join(BITJOIN,219)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,306)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
