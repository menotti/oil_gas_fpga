...
// SystemVerilog created from k2_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE2800002_function_cra_slave
...
module k2_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE2800002_function_cra_slave (
    ...
    output wire [1983:0] kernel_arguments,
    ...
    wire [1983:0] kernel_arg_bit_join_q;
    ...
    // printf_bit(BITSELECT,259)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,260)
    ...
    // not_last_finish_state(LOGICAL,227)
    ...
    // readdata_valid_stage1_reg(REG,267)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_was_finish_counter_address_stage1_reg(REG,157)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // can_write(LOGICAL,140)
    assign can_write_q = NO_NAME_q & avs_cra_write;

    // status_select(MUX,293)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,158)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,155)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,238)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,268)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,156)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_6_arg_bit_join_x(BITJOIN,339)
    ...
    // dupName_5_arg_bit_join_x(BITJOIN,331)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,300)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
