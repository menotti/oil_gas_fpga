...
function updateTripCountTree() {
  ...
  delete(loopDataNodes);
  delete(loopDataNodesPassedInSaved);
  delete(loopLegacyNodes);
  delete(bottleneckNodes);
  delete(invocationMap);
  delete(gantt_chart);
  delete(outerMostLoopNodesList);
  delete(taskNodePairsWithTripCounts);
  delete(tripCountNodeMap);
...
function createImplicitLimiter(startNode, endNode, startPoint, endPoint, capacity, checkPreviousIteration, parentNode) {
  ...
  if (Object.entries(parentNode).length > 0)
    addLimiterToLoopNodes(parentNode, startNode, endNode, limiterStartElement, limiterEndElement, capacity, checkPreviousIteration);
  else 
    addLimiterToLoopNodes(startNode, startNode, endNode, limiterStartElement, limiterEndElement, capacity, checkPreviousIteration);
...
function addLimiterToLoopNodes(limiterParentNode,
                       ...
                       limitedRegionCapacity, checkPreviousIteration) {
  if (checkPreviousIteration === undefined) {
    checkPreviousIteration = false;
  }
      ...
      for (let j = 0; j < loopNode.children.length; j++) {
        ...
        if (childLoopNode.type == "loop" && (childLoopNode.pipelined == "Yes" || childLoopNode.pipelined == 1)) {
          ...
          if (parseInt(childLoopNode.interleaving) > 1) {
            ...
            createImplicitLimiter(predecessorNode, childLoopNode, predecessorNode.latency, childLoopNode.ii, childLoopNode.interleaving, false, parentNode);
            ...
            createImplicitLimiter(predecessorNode, childLoopNode, predecessorNode.latency, childLoopNode.ii, 1, false, parentNode);
          }
        }
      }
...
function initializeTripCounts(constValue, loopNodeIDTripCountPairs) {
  ...
  for (let i = 0; i < loopNodesToAnalyze.length; i++) {
    ...
    if (loopNodeObj.hasOwnProperty("loop_node") && loopNodeObj.hasOwnProperty("trip_count")) {
      ...
      if (!isNaN(loopNodeTripCount) && loopNodeTripCount > 0) tripCount = loopNodeTripCount;
    }
