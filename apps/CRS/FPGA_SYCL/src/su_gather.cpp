...
su_gather::su_gather(std::string& file_path, int aph, int apm, int nc) :
...
//    tr.fgettr(file);
  ...
  this->_cdps_by_cdp_id.resize(_cdps.size());

  this->_max_gather = 0;
  for(int i=0; i < _cdps.size(); i++) {
    int n_gathers = 0;
    float m0x = _cdps[i].traces()[0].midpoint_x();
    float m0y = _cdps[i].traces()[0].midpoint_y();

    for(int j=0; j < _cdps.size(); j++) {
      float dx = _cdps[j].traces()[0].midpoint_x() - m0x;
      float dy = _cdps[j].traces()[0].midpoint_y() - m0y;

      if(dx*dx + dy*dy <= apm * apm) {
        _cdps_by_cdp_id[i].push_back(j);
        n_gathers++;
      }
    }

    if(n_gathers > _max_gather) _max_gather = n_gathers;
  }
