...
int main(int argc, const char** argv) {
	...
	try {
		...
		LOG(DEBUG, "Starting FPGA SYCL devices");
...
/*		  real m0x_cdp_id = m0x[0], m0y_cdp_id = m0y[0];
		...
		for(int cdp_id=0; cdp_id < ncdps; cdp_id++) {
			...
			std::cout << "Kernel 3 no cdp_id: " << cdp_id << std::endl;			
			...
			std::cout << "Kernel 4 no cdp_id: " << cdp_id << std::endl;	
			sycl::buffer<real, 1> b_cdpsmpl(cdpsmpl, sycl::range<1>(ntrs * max_gather * ns));
			...
			queue.submit([&](sycl::handler& cgh) {
				...
				auto a_m2      = b_m2.get_access<sycl::access::mode::read_write>(cgh);
				auto a_m       = b_m.get_access<sycl::access::mode::read_write>(cgh);
				auto a_samples = b_cdpsmpl.get_access<sycl::access::mode::read_write>(cgh);
			...
			std::cout << "Kernel 5 no cdp_id: " << cdp_id << std::endl;	
