...
// SystemVerilog created from k1_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE1600000_function_cra_slave
...
module k1_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE1600000_function_cra_slave (
    ...
    output wire [1567:0] kernel_arguments,
    ...
    wire [1567:0] kernel_arg_bit_join_q;
    ...
    // printf_counter_reset_mux(MUX,235)
    ...
    // printf_bit(BITSELECT,233)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,234)
    ...
    // readdata_valid_stage1_reg(REG,241)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,137)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,132)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,129)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,212)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,242)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,130)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // done_or_printf_or_profile_irq_signal(LOGICAL,135)
    ...
    // global_offset_reg_upper_0(REG,153)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,147)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,155)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,149)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,157)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,151)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0_buffered(REG,163)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,162)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,164)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,166)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_37_address_ref_x(CONSTANT,532)
    ...
    // dupName_35_address_ref_x(CONSTANT,520)
    ...
    // dupName_22_can_write_x(LOGICAL,442)
    ...
    // dupName_18_address_ref_x(CONSTANT,408)
    ...
    // dupName_1_arg_bit_join_x(BITJOIN,283)
    ...
    // dupName_9_can_write_x(LOGICAL,338)
    ...
    // local_size_reg_0_buffered(REG,190)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_0_buffered(REG,206)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_1_buffered(REG,208)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_2_buffered(REG,210)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
