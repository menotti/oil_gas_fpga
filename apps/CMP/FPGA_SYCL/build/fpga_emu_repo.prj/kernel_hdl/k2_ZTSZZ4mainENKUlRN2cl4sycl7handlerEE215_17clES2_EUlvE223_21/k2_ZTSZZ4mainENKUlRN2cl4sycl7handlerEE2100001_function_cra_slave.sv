...
// SystemVerilog created from k2_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE2100001_function_cra_slave
...
module k2_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE2100001_function_cra_slave (
    ...
    output wire [1535:0] kernel_arguments,
    ...
    wire [1535:0] kernel_arg_bit_join_q;
    ...
    // readdata_valid_stage1_reg(REG,239)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,135)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // can_write(LOGICAL,112)
    assign can_write_q = NO_NAME_q & avs_cra_write;

    // status_select(MUX,265)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,130)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,127)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,210)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,240)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,128)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdatavalid(GPOUT,211)
    assign avs_cra_readdatavalid = cra_output_readdatavalid_reg_q;

    // dupName_0_ctrl_profile_status_bit_x(BITSELECT,276)
    ...
    // status_done_bit(BITSELECT,261)
    assign status_done_bit_b = status_NO_SHIFT_REG_q[1:1];

    // done_or_printf_or_profile_irq_signal(LOGICAL,133)
    ...
    // global_offset_reg_upper_0(REG,151)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,145)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,153)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,147)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,155)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,149)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,160)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,162)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,164)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_14_address_ref_x(CONSTANT,374)
    assign dupName_14_address_ref_x_q = $unsigned(6'b001111);

    // dupName_14_NO_NAME_x(LOGICAL,373)
    ...
    // local_size_reg_1_buffered(REG,190)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // work_dim_NO_SHIFT_REG_buffered(REG,271)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,272)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
